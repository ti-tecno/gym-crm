  ...
  return (
    <div style={{
        ...
        {NAV_ITEMS.map((item) => (
          <NavLink
            ...
            style={({ isActive }) => ({
              ...
              fontWeight: isActive ? 700 : 500, transition: 'all 0.15s', textAlign: 'left',
              textDecoration: 'none',
            })}
          >
            {({ isActive }) => (
              <>
                ...
                {item.label}
                {isActive && <div style={{ width: 3, height: 14, background: COLORS.accent, borderRadius: 2, marginLeft: 'auto' }} />}
              </>
            )}
          </NavLink>
        ))}
      ...
      <div style={{ padding: '12px 14px', borderTop: `1px solid ${COLORS.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
        <Avatar initials={initials(user?.nombre || 'Usuario')} size={30} color={COLORS.blue} />

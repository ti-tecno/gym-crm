...
export default function ClienteSidebar() {
  ...
  return (
    <div style={{
        ...
        {CLIENTE_NAV_ITEMS.map((item) => (
          <NavLink key={item.id} to={item.to}
            style={({ isActive }) => ({
              ...
              fontWeight: isActive ? 700 : 500, transition: 'all 0.15s', textAlign: 'left', textDecoration: 'none',
            })}>
            {({ isActive }) => (
              <>
                <span style={{ opacity: isActive ? 1 : 0.6 }}>{item.icon}</span>
                {item.label}
                {isActive && <div style={{ width: 3, height: 14, background: COLORS.green, borderRadius: 2, marginLeft: 'auto' }} />}
              </>
            )}
          </NavLink>
        ))}
      ...
      <div style={{ padding: '12px 14px', borderTop: `1px solid ${COLORS.border}`, display: 'flex', alignItems: 'center', gap: 10 }}>
        <Avatar initials={initials(user?.nombre || 'Cliente')} size={30} color={COLORS.green} />
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ color: COLORS.text, fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {user?.nombre || 'Cliente'}
          </div>
          <div style={{ color: COLORS.muted, fontSize: 10 }}>CLIENTE</div>
        </div>
        <button onClick={handleLogout} title="Cerrar sesión"
          style={{ background: 'transparent', color: COLORS.muted, border: `1px solid ${COLORS.border}`, borderRadius: 6, padding: '4px 8px', fontSize: 11, cursor: 'pointer' }}>
          ⎋
        </button>
      </div>
    </div>

...
const PLANS = [
  {
    nombre: 'Básico',
    precio: 450,
    features: ['Acceso a todas las máquinas', 'Vestidores y casilleros', 'Horario completo', 'Evaluación inicial'],
    color: COLORS.blue,
    destacado: false,
  },
  {
    nombre: 'Premium',
    precio: 850,
    features: ['Todo lo del plan Básico', 'Clases grupales ilimitadas', 'App de seguimiento', 'Asesoría nutricional básica'],
    color: COLORS.accent,
    destacado: true,
  },
  {
    nombre: 'Elite',
    precio: 1200,
    features: ['Todo lo del plan Premium', 'Coach personal dedicado', 'Plan nutricional personalizado', 'Suplementos con 10% dto'],
    color: COLORS.purple,
    destacado: false,
  },
];
...
const GALLERY = [
  { src: '/images/gym-1.jpg', alt: 'Área de pesas libres',  path: 'frontend/public/images/gym-1.jpg' },
  { src: '/images/gym-2.jpg', alt: 'Zona cardio',           path: 'frontend/public/images/gym-2.jpg' },
  { src: '/images/gym-3.jpg', alt: 'Zona funcional',        path: 'frontend/public/images/gym-3.jpg' },
  { src: '/images/gym-4.jpg', alt: 'Sala de máquinas',      path: 'frontend/public/images/gym-4.jpg' },
  { src: '/images/gym-5.jpg', alt: 'Área de musculación',   path: 'frontend/public/images/gym-5.jpg' },
  { src: '/images/gym-6.jpg', alt: 'Área CrossFit',         path: 'frontend/public/images/gym-6.jpg' },
];
...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    const N = isMobile
      ? 40
      : Math.min(100, Math.floor((window.innerWidth * window.innerHeight) / 11000));
    ...
    const draw = () => {
      ...
      pts.forEach(p => {
        ...
        p.vx = Math.max(-1.8, Math.min(1.8, p.vx * .985));
        p.vy = Math.max(-1.8, Math.min(1.8, p.vy * .985));
      ...
      for (let i = 0; i < pts.length; i++) {
        for (let j = i + 1; j < pts.length; j++) {
          ...
          const d  = Math.sqrt(dx * dx + dy * dy);
          if (d < 155) {
            ctx.strokeStyle = `rgba(245,166,35,${(1 - d / 155) * .22})`;
            ctx.lineWidth   = .55;
            ctx.beginPath();
  ...
  useEffect(() => {
    const reveal = () =>
      document.querySelectorAll('.ic-reveal, .ic-reveal-l, .ic-reveal-r').forEach(el => {
  ...
  return (
    <div style={{ background: COLORS.bg, color: COLORS.text, fontFamily: 'Inter, sans-serif', overflowX: 'hidden' }}>
      ...
      <nav style={{
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, cursor:'pointer' }} onClick={() => scrollTo('hero')}>
          <img
            src="/images/logo.jpg"
            alt="Iron Core"
            style={{ height:36, width:36, borderRadius:8, objectFit:'cover', flexShrink:0, boxShadow:`0 4px 18px ${COLORS.accent}55` }}
          />
          <span style={{
            fontFamily:'Barlow Condensed, sans-serif',
            fontWeight:800, fontSize:20, letterSpacing:3, color: COLORS.text,
          }}>IRON CORE</span>
        </div>

        {/* Desktop links */}
        <div className="ic-nav-desktop" style={{ alignItems:'center', gap:28 }}>
          {[['NOSOTROS','features'],['HORARIO','horario'],['PLANES','planes'],['GALERÍA','galeria']].map(([lbl,id]) => (
            <span key={id} className="ic-nav-link" onClick={() => scrollTo(id)}
              style={{ fontFamily:'Barlow Condensed, sans-serif', fontSize:13, fontWeight:600, letterSpacing:2, color: COLORS.muted }}>
              {lbl}
            </span>
          ))}
          <button className="ic-btn" onClick={() => navigate('/login')} style={{
            background:`linear-gradient(135deg,${COLORS.accent},${COLORS.accentDim})`,
            color: COLORS.bg, border:'none', padding:'10px 24px', borderRadius:8,
            fontFamily:'Barlow Condensed, sans-serif', fontWeight:800, fontSize:13,
            letterSpacing:2, boxShadow:`0 4px 22px ${COLORS.accent}45`,
          }}>ENTRAR</button>
        </div>
        ...
        <button
          ...
          style={{
            background:'transparent', border:`1px solid ${menuOpen ? COLORS.accent : COLORS.border}`,
        ...
        >
          {menuOpen ? (
            /* X icon */
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" stroke={COLORS.accent} strokeWidth="2" strokeLinecap="round">
              <line x1="4" y1="4" x2="16" y2="16"/><line x1="16" y1="4" x2="4" y2="16"/>
            </svg>
          ) : (
            /* Hamburger icon */
            <svg width="20" height="16" viewBox="0 0 20 16" fill="none" stroke={COLORS.text} strokeWidth="2" strokeLinecap="round">
      ...
      <div className={`ic-mobile-menu${menuOpen ? ' open' : ''}`}>
        ...
        <div style={{ position:'absolute', inset:0 }} onClick={() => setMenuOpen(false)} />

        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
          <img src="/images/logo.jpg" alt="Iron Core"
            style={{ height:44, width:44, borderRadius:10, objectFit:'cover', boxShadow:`0 4px 18px ${COLORS.accent}55` }}
          />
          <span style={{ fontFamily:'Barlow Condensed, sans-serif', fontWeight:800, fontSize:24, letterSpacing:3, color: COLORS.text }}>
            IRON CORE
          </span>
        </div>

        <div style={{ width:40, height:2, background:`${COLORS.accent}`, borderRadius:2, marginBottom:8 }} />

        {[['NOSOTROS','features'],['HORARIO','horario'],['PLANES','planes'],['GALERÍA','galeria']].map(([lbl,id]) => (
          <button key={id} onClick={() => { scrollTo(id); setMenuOpen(false); }} style={{
            background:'transparent', border:'none', cursor:'pointer',
            fontFamily:'Barlow Condensed, sans-serif', fontWeight:700,
            fontSize:28, letterSpacing:4, color: COLORS.subtle,
        ...
        <button className="ic-btn" onClick={() => navigate('/login')} style={{
          marginTop:8,
          background:`linear-gradient(135deg,${COLORS.accent},${COLORS.accentDim})`,
          color: COLORS.bg, border:'none', padding:'16px 48px', borderRadius:12,
          fontFamily:'Barlow Condensed, sans-serif', fontWeight:800, fontSize:20,
          letterSpacing:3, boxShadow:`0 8px 30px ${COLORS.accent}50`,
      ...
      <section id="hero" style={{
        ...
        <div style={{
          ...
          animation:'ic-scan 5s linear infinite', pointerEvents:'none',
        }} />
        ...
        <div style={{
          ...
          filter:'blur(50px)', animation:'ic-float 7s ease-in-out infinite', pointerEvents:'none',
        }} />
        <div style={{
          position:'absolute', bottom:'18%', right:'6%',
          width:360, height:360, borderRadius:'50%',
          background:`radial-gradient(circle, ${COLORS.blue}14, transparent 70%)`,
          filter:'blur(70px)', animation:'ic-float2 9s ease-in-out infinite', pointerEvents:'none',
        }} />
        <div style={{
          position:'absolute', top:'40%', right:'15%',
          width:160, height:160, borderRadius:'50%',
          background:`radial-gradient(circle, ${COLORS.purple}12, transparent 70%)`,
          filter:'blur(40px)', animation:'ic-float 11s ease-in-out infinite reverse', pointerEvents:'none',
        }} />
        ...
        <div style={{
          ...
          animation:'ic-float 8s ease-in-out infinite', pointerEvents:'none',
        }} />
        <div style={{
          position:'absolute', bottom:'25%', right:'10%',
          width:60, height:60, borderRadius:'50%',
          border:`1px solid ${COLORS.blue}30`,
          animation:'ic-float2 6s ease-in-out infinite', pointerEvents:'none',
        }} />

        {/* Content */}
        <div style={{ opacity: heroOpacity, transform:`translateY(${heroY}px)`, padding:'0 24px', maxWidth:1000 }}>

          {/* Badge */}
          <div style={{
            display:'inline-flex', alignItems:'center', gap:8,
            background:`${COLORS.accent}12`, border:`1px solid ${COLORS.accent}35`,
            ...
            <span style={{
              fontFamily:'DM Mono, monospace', fontSize:10, letterSpacing:4,
              color: COLORS.accent, textTransform:'uppercase',
            }}>BIENVENIDO A IRON CORE</span>
          </div>

          {/* IRON — 3D extrusion via stacked text-shadow */}
          <h1 style={{
            fontFamily:'Barlow Condensed, sans-serif',
          ...
          <h1 style={{
            ...
            fontSize:'clamp(80px, 16vw, 175px)',
            fontWeight:800, lineHeight:.88,
            margin:'0 0 36px', letterSpacing:'-3px', textTransform:'uppercase',
            background:`linear-gradient(135deg, ${COLORS.accent} 0%, #FFE066 45%, ${COLORS.accent} 100%)`,
            backgroundSize:'220% auto',
            WebkitBackgroundClip:'text', backgroundClip:'text',
            WebkitTextFillColor:'transparent',
            animation:'ic-shimmer 3s linear infinite, ic-slide-up .7s .2s ease both',
          }}>CORE</h1>

          {/* Tagline */}
          <p style={{
            fontFamily:'Barlow Condensed, sans-serif',
            fontSize:'clamp(16px, 2.8vw, 26px)', fontWeight:600,
            letterSpacing:7, color: COLORS.subtle, textTransform:'uppercase',
            margin:'0 0 48px',
            animation:'ic-slide-up .7s .3s ease both',
          }}>FORJA TU MEJOR VERSIÓN</p>

          {/* CTA buttons */}
          <div style={{ display:'flex', gap:16, justifyContent:'center', flexWrap:'wrap', animation:'ic-slide-up .7s .4s ease both' }}>
            <button className="ic-btn" onClick={() => navigate('/login')} style={{
              background:`linear-gradient(135deg,${COLORS.accent},${COLORS.accentDim})`,
              color: COLORS.bg, border:'none', padding:'16px 44px', borderRadius:14,
            ...
            <button className="ic-outline-btn" onClick={() => scrollTo('features')} style={{
              background:'transparent', color: COLORS.subtle,
              border:`1px solid ${COLORS.border}`, padding:'16px 44px', borderRadius:14,
        ...
        <div style={{
          ...
          <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke={COLORS.accent} strokeWidth="1.8">
      ...
      <div style={{
        ...
        background:`${COLORS.surface}cc`, backdropFilter:'blur(10px)',
        ...
        <div style={{
          ...
          {[...Array(3)].map((_, i) => (
            <span key={i} style={{
              fontFamily:'Barlow Condensed, sans-serif', fontWeight:700,
              fontSize:13, letterSpacing:5, color: COLORS.accent,
              textTransform:'uppercase', paddingRight:0, whiteSpace:'nowrap',
            }}>
      ...
      <section ref={statsRef} style={{
        ...
        <div style={{
          ...
          {STATS.map((stat, i) => (
            <div key={i} className="ic-reveal" style={{
              ...
              }}>{counts[i]}{stat.suffix}</div>
              <div style={{
                fontFamily:'DM Mono, monospace', fontSize:11,
                letterSpacing:2, color: COLORS.muted, textTransform:'uppercase', marginTop:8,
              }}>{stat.label}</div>
            </div>
          ))}
        </div>
      ...
      {/* ── FEATURES ───────────────────────────────────────── */}
      <section id="features" style={{ position:'relative', zIndex:1, padding:'100px 48px' }} className="ic-section-pad">
        <div style={{ maxWidth:1100, margin:'0 auto' }}>

          <div className="ic-reveal" style={{ textAlign:'center', marginBottom:64 }}>
            <p style={{ fontFamily:'DM Mono, monospace', fontSize:10, letterSpacing:4, color: COLORS.accent, textTransform:'uppercase', marginBottom:12 }}>
              POR QUÉ ELEGIRNOS
            </p>
            <h2 style={{
              fontFamily:'Barlow Condensed, sans-serif',
            ...
            }}>LA EXPERIENCIA<br/>IRON CORE</h2>
          </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(300px, 1fr))', gap:24 }}>
            {FEATURES.map((feat, i) => (
              ...
              >
                <div className="ic-shine" style={{ position:'absolute', inset:0, borderRadius:22, pointerEvents:'none', transition:'background .3s' }} />
                <div style={{
                ...
                <div style={{ fontSize:50, marginBottom:22, filter:`drop-shadow(0 0 22px ${feat.color}65)` }}>{feat.icon}</div>

                <h3 style={{
                  fontFamily:'Barlow Condensed, sans-serif',
                  fontSize:28, fontWeight:800, textTransform:'uppercase',
                  letterSpacing:1, color: COLORS.text, margin:'0 0 12px',
                }}>{feat.title}</h3>

                <p style={{ color: COLORS.muted, lineHeight:1.75, fontSize:15, margin:0 }}>{feat.desc}</p>
      ...
      {/* ── GALLERY ────────────────────────────────────────── */}
      <section id="galeria" style={{ position:'relative', zIndex:1, padding:'100px 48px' }} className="ic-section-pad">
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          ...
          <div className="ic-reveal" style={{ textAlign:'center', marginBottom:64 }}>
            ...
            <h2 style={{
              ...
              textTransform:'uppercase', letterSpacing:'-1px',
              color: COLORS.text, margin:0,
            }}>DONDE SE FORJAN<br/>LOS CAMPEONES</h2>
          </div>

          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(340px, 1fr))', gap:16 }}>
            {GALLERY.map((img, i) => (
              <div key={i} className={`ic-gal-item ic-reveal`} style={{
                borderRadius:18, aspectRatio:'16/10', position:'relative',
                background:`linear-gradient(135deg, ${COLORS.surface}, ${COLORS.card})`,
                border:`1px solid ${COLORS.border}`,
                transitionDelay:`${(i % 3) * .1}s`,
              }}>
                ...
                <div style={{
                  ...
                  <div style={{
                    ...
                    fontSize:20, color: COLORS.border,
                  }}>📸</div>
                  <code style={{
                    fontFamily:'DM Mono, monospace', fontSize:10,
                    color: COLORS.muted, letterSpacing:.5, textAlign:'center', padding:'0 12px',
                  }}>{img.path}</code>
                  <span style={{ fontFamily:'Inter, sans-serif', fontSize:12, color: COLORS.subtle }}>{img.alt}</span>
                </div>
                ...
                <div style={{
                  ...
                  <span style={{
                    fontFamily:'Barlow Condensed, sans-serif',
                    fontSize:15, fontWeight:700, letterSpacing:1, color: COLORS.text,
                  }}>{img.alt}</span>
                </div>
              </div>
            ))}
      ...
      {/* ── SCHEDULE ───────────────────────────────────────── */}
      <section id="horario" style={{ position:'relative', zIndex:1, padding:'100px 48px' }} className="ic-section-pad">
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
                ...
                <tr>
                  ...
                  {DAYS.map(day => (
                    <th key={day} style={{
                      ...
                      color: COLORS.bg, textAlign:'center',
                    }}>{day}</th>
                  ))}
                </tr>
              </thead>
          ...
          {/* Legend */}
          <div className="ic-reveal" style={{
            marginTop:24, display:'flex', flexWrap:'wrap', gap:12, justifyContent:'center',
          }}>
            {Object.entries(CLASS_COLORS).map(([name, color]) => (
              <div key={name} style={{
                ...
                <span style={{ fontFamily:'Barlow Condensed, sans-serif', fontWeight:700, fontSize:13, letterSpacing:1, color: COLORS.subtle }}>
                  {name}
                </span>
              </div>
            ))}
      ...
      {/* ── PLANS ──────────────────────────────────────────── */}
      <section id="planes" style={{ position:'relative', zIndex:1, padding:'100px 48px' }} className="ic-section-pad">
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(290px, 1fr))', gap:24, alignItems:'start' }}>
            {PLANS.map((plan, i) => (
              <div key={i} className="ic-plan-card ic-reveal" style={{
                ...
                position:'relative', overflow:'hidden',
                transitionDelay:`${i * .1}s`,
                boxShadow: plan.destacado ? `0 0 50px ${plan.color}28` : 'none',
                animation: plan.destacado ? 'ic-box-glow 3.5s ease-in-out infinite' : 'none',
              }}>
                {plan.destacado && (
                  <div style={{
                    position:'absolute', top:16, right:-32,
                    background:`linear-gradient(135deg,${COLORS.accent},${COLORS.accentDim})`,
                    color: COLORS.bg, padding:'4px 44px',
                    fontFamily:'Barlow Condensed, sans-serif',
                ...
                <div style={{ marginBottom:24 }}>
                  <div style={{ width:44, height:3, background:`linear-gradient(90deg,${plan.color},transparent)`, borderRadius:2, marginBottom:16 }} />
                  <h3 style={{
                    fontFamily:'Barlow Condensed, sans-serif', fontSize:28, fontWeight:800,
                    textTransform:'uppercase', letterSpacing:2, color: plan.color, margin:'0 0 10px',
                  }}>{plan.nombre}</h3>
                  <div style={{ display:'flex', alignItems:'baseline', gap:4 }}>
                    <span style={{ color: COLORS.muted, fontFamily:'Barlow Condensed, sans-serif', fontSize:20 }}>$</span>
                    <span style={{
                      fontFamily:'Barlow Condensed, sans-serif', fontSize:62, fontWeight:800,
                      color: COLORS.text, lineHeight:1,
                    }}>{plan.precio}</span>
                    <span style={{ color: COLORS.muted, fontSize:13 }}>/mes</span>
                  </div>
                </div>
                    ...
                    <li key={j} style={{ display:'flex', alignItems:'center', gap:10 }}>
                      <div style={{
                        ...
                        <svg width="10" height="8" viewBox="0 0 10 8" fill="none">
                          <path d="M1 4L3.5 6.5L9 1" stroke={plan.color} strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                      </div>
                      <span style={{ color: COLORS.subtle, fontSize:14, lineHeight:1.4 }}>{f}</span>
                    </li>
                  ))}
                </ul>
                ...
                <button className="ic-btn" onClick={() => navigate('/login')} style={{
                  ...
                  borderRadius:12,
                  border: plan.destacado ? 'none' : `1px solid ${plan.color}50`,
                  background: plan.destacado
                  ...
                  boxShadow: plan.destacado ? `0 8px 28px ${plan.color}45` : 'none',
      ...
      {/* ── FINAL CTA ──────────────────────────────────────── */}
      <section style={{
        position:'relative', zIndex:1, padding:'120px 48px', textAlign:'center',
        ...
        <div className="ic-reveal" style={{ maxWidth:700, margin:'0 auto' }}>
          <p style={{ fontFamily:'DM Mono, monospace', fontSize:10, letterSpacing:4, color: COLORS.accent, textTransform:'uppercase', marginBottom:16 }}>
            ¿LISTO PARA EMPEZAR?
          </p>
          <h2 style={{
            fontFamily:'Barlow Condensed, sans-serif',
            fontSize:'clamp(56px, 10vw, 110px)', fontWeight:800,
            textTransform:'uppercase', letterSpacing:'-3px', lineHeight:.9,
            color: COLORS.text, margin:'0 0 20px',
            textShadow:`0 0 80px ${COLORS.accent}18`,
          }}>EL HIERRO<br/>TE ESPERA</h2>
          <p style={{ color: COLORS.muted, fontSize:16, lineHeight:1.75, margin:'0 0 48px' }}>
            Únete a Iron Core hoy y comienza tu transformación.<br/>
            Cada rep cuenta. Cada día importa.
          </p>
          <button className="ic-btn" onClick={() => navigate('/login')} style={{
            background:`linear-gradient(135deg,${COLORS.accent},${COLORS.accentDim})`,
            color: COLORS.bg, border:'none', padding:'20px 60px', borderRadius:16,
      ...
      <footer style={{
        ...
        background:`${COLORS.surface}99`, backdropFilter:'blur(12px)',
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10 }}>
          ...
          <span style={{ fontFamily:'Barlow Condensed, sans-serif', fontWeight:800, fontSize:16, letterSpacing:2, color: COLORS.text }}>
            IRON CORE
          </span>
        </div>
        <span style={{ fontFamily:'DM Mono, monospace', fontSize:10, color: COLORS.muted, letterSpacing:1 }}>
        ...
        <button
          ...
          style={{
            ...
            color: COLORS.muted, padding:'8px 22px', borderRadius:8,
